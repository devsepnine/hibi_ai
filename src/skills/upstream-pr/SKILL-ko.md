...
- **로컬 파일이 모두 이 저장소에서 온 것은 아니다.** `~/.hibi/sources.yaml` 로 다른 git·로컬 소스를 추가할 수 있고, 사용자가 직접 만든 스킬도 있다. 이 저장소의 설치 표면 밖에 있는 것은 아무리 좋아도 후보가 아니다 — 로컬 전용 파일을 drift로 취급하기 전에 소스 목록을 확인한다.
