...
Result, Option, error 라이브러리를 사용한 Rust의 에러 핸들링 종합 가이드.
...
|---------|----------|---------|
| `Result<T, E>` | 복구 가능한 에러 | File I/O, parsing |
| `Option<T>` | 값 부재 | 컬렉션에서 항목 찾기 |
| `panic!` | 복구 불가능한 에러 | 프로그래밍 버그, invariant 위반 |
| `thiserror` | 라이브러리 에러 | 커스텀 에러 타입 정의 |
| `anyhow` | 애플리케이션 에러 | 컨텍스트 추가, 에러 핸들링 단순화 |
| `?` operator | 에러 전파 | 작업 chain |
| `.context()` | 정보 추가 | 에러 컨텍스트 제공 |
| `.map_err()` | 에러 변환 | 에러 타입 변환 |
| `.unwrap_or()` | 기본값 제공 | non-critical 실패 |
