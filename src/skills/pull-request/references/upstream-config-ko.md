...
아니라 증거와 영향 범위다. 무엇이 실패했고 누구에게 도달하는지 모르면 리뷰어는
규칙 변경을 판단할 수 없다.

`pull-request` skill의 설정 기여 케이스다. 제목·본문·브랜치 규약은 그 스킬에서 오고,
아래는 이 케이스가 그 위에 더하는 것이다.
...
이 워크플로우를 실행하라는 요청 자체가 베이스 권한 규칙이 요구하는 명시적 요청이고,
로컬 브랜치와 커밋까지를 승인한다 — `improve/<topic>`, 커밋은 논리 단위로 하나씩,
`main` 직커밋은 하지 않는다. 공개 업스트림은 베이스 규칙보다 **더** 요구한다: push와
PR 생성에는 *두 번째* 확인이 따로 필요하고, 워크플로우를 실행한 것만으로는 승인되지
않는다.

설정 변경에는 티켓이 없으므로 제목은 요약만 쓴다 — 티켓 접두사는 생략한다. `SKILL.md`
의 제목 규칙에 따른다.
