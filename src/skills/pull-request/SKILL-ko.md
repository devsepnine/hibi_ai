...
나가는 행위다 — 사람들에게 알림이 가고 브랜치가 공개된다. 커밋과 같은 규칙으로, 그 푸시나
PR에 대한 명시적 요청이 있을 때만 한다. 그 요청은 이월되지 않는다: 이전의 "알아서 해줘",
직전 푸시에 대한 승인, 승인받은 계획, 명령을 자동 승인하는 권한 모드는 어느 것도 그 요청이
아니다. 요청이 없으면 초안만
보여주고 멈춘다. 공개 저장소로 나가는 경우는 문턱이 더 높다 —
