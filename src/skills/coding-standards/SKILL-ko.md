...
주석이 *무엇*을 설명하고 있다면, 해법은 더 나은 주석이 아니라 리팩토링이다:

```typescript
// Bad — 스스로 말하지 못하는 코드를 주석으로 떠받친다
// 사용자가 이 마켓을 수정할 수 있는지 확인
if (user.role === 'admin' || (market.ownerId === user.id && !market.closed)) {

// Good — 이름이 명세를 담는다. 주석이 필요 없다
...
// 한 선언에 doc 블록이 두 개 쌓이고 자식의 실제 계약이
// 호출자를 설명하는 문장 아래에 묻힌다
/** 마켓 스냅샷을 조회한다. 캐시가 유효하면 캐시에서 반환한다. */
/** 마켓의 캐시 키. 피드 버전으로 네임스페이스를 분리한다. */
function cacheKeyFor(marketId: string): string {

// Good — 각 함수가 자기 계약만 갖는다
/** 마켓의 캐시 키. 피드 버전으로 네임스페이스를 분리한다. */
function cacheKeyFor(marketId: string): string {
```
