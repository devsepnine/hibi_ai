...
- 복잡하거나 불명확한 문제는 먼저 **Problem 1-Pager** 작성: Background / Problem / Goal / Non-goals / Constraints — 항목이 모호하면 인터뷰 요청

### 2. 서브에이전트 전략
- 메인 컨텍스트 윈도우를 깨끗하게 유지하기 위해 서브에이전트 활용 — 단 Opus 5는 기본 위임 성향이 강하므로 부추기지 않는다: 도구 호출 몇 번이면 직접 끝낼 일에는 스폰하지 않기
- 리서치, 탐색, 병렬 분석은 서브에이전트에 위임 — 위임했으면 결과를 다시 파헤치지 말고 그대로 활용
- 복잡한 문제에는 서브에이전트를 통해 더 많은 컴퓨팅 투입
- 집중된 실행을 위해 서브에이전트당 하나의 작업만 배정
- 팀 작업 종합이 끝나면 더 쓸 일 없는 팀메이트에게 shutdown을 요청해 정리 — idle 팀메이트는 리드 세션 종료까지 살아 있음 (토큰 비용은 없지만 프로세스 유지)
...
- 이 프로젝트를 넘어 일반화되는 교훈은 배포 설정에 반영하자고 먼저 제안하고, 사용자가 동의하면 `/upstream-pr` 을 실행한다. 개인적인 것은 `MEMORY.md` 에 남긴다
...
- 테스트 실행, 로그 확인, 정확성 입증
- **작업 후 필수 리뷰**: 코드/콘텐츠 변경 후 완료 보고 전에 diff를 리뷰한다 — 변경 파일에 `code-reviewer` 에이전트(또는 `/code-review`)를 실행하고, 의존성·결합도·모듈·모노레포 변경은 `dependency-design` skill도 적용한다. 각 발견사항은 반영하거나 명시적으로 보류한다. 순수 대화나 사소한 비코드 편집만 예외로 한다.
...
**핵심 원칙**: "프롬프트 주변을 길게 쓰기보다 effort를 올려라." Opus 5는 effort를 엄격히 따른다. 더 낮은 effort에서 요청된 것만 한다 — 그 이상은 안 한다. Opus 5의 `low`/`medium`은 이전 세대 대비 체감상 훨씬 강력하므로, 평가로 품질이 유지되면 낮춰 쓴다.

**낮은 effort에서의 도구 사용**: 도구 호출을 결합하고 적게 사용. 직접 행동 → 간결한 확인.
**높은 effort에서의 도구 사용**: 행동 전 계획 설명, 더 많은 도구 호출, 상세 요약.

**모델 선택**: 빈번한 경량 워커는 Haiku 4.5, 메인 개발·도구 중심 작업은 Sonnet 5, 깊은 추론·장시간(30분+) 에이전트 작업은 Opus 5. Fable 5는 최고난도 프론티어 작업에 명시적으로 선택할 때만 (기본 업그레이드 경로 아님).
...
재작업. 상세 체크리스트는 `coding-standards` skill의 `references/review-checklist.md` 참조.
...
- **코드가 곧 명세다**: *무엇*은 이름·타입·구조가 말하고, 주석은 코드가 표현할 수 없는 *왜*만 담는다. 절대 규칙은 아래 "절대 주석 규칙", 상세는 `coding-standards` skill.

### 의사결정 원칙
- **두 가지 대안 이상 검토**: 첫 번째 솔루션이 최선이 아닐 수 있음
- **트레이드오프 명시**: 선택한 방법의 장단점 인지
- **되돌릴 수 있는가**: 위험한 작업은 반드시 사용자 확인
- **작게 유지**: 작업·커밋·PR을 작게. 가정은 Issue/PR/ADR에 기록
- **입력 검증·출력 인코딩**: 검증 안 된 입력을 신뢰하지 말 것
- **이름이 추상화보다 우선**: 의도를 드러내는 이름, 성급한 추상화 금지

## 절대 커밋 규칙 (항상 적용 — 상세는 skill이 담당)

`commit-rules` skill이 로드되기 전이라도 커밋 시 다음은 절대 불가:
- **이모지·생성 마커 금지** (`Co-Authored-By`, "Generated with Claude Code" 등)
- **사용자가 명시적으로 요청할 때만 커밋.** 작업 끝났다고 자동 커밋 금지.
- 형식: `<type>: [<ticket>] <title>` — 전체 컨벤션은 `commit-rules` skill 참조.

## 절대 주석 규칙 (항상 적용 — 상세는 skill이 담당)

**코드가 곧 명세다.** *무엇*을 하는지는 이름·타입·구조가 말한다. 주석은 코드가 표현할 수 없는 것 — *왜*: 의도, 제약, 트레이드오프, 불변식, 외부 맥락 — 에만 존재한다. `coding-standards` skill이 로드되기 전이라도, 코드를 쓰거나 고칠 때 다음은 절대 규칙이다:

- **'무엇'을 설명하는 주석 → 주석 대신 리팩토링** (이름 변경, 함수/상수 추출). 코드를 되풀이하는 주석은 문서가 아니라 결함이다.
- **과잉 주석 금지**: 기본값은 '주석 없음' — 의미 없는 주석은 정작 중요한 주석을 묻어버리는 잡음이다.
- **절대 금지**: 코드 나열식 서술, 낡은 주석, 주석 처리된 코드, 변경 이력 주석(`// 2026-01-02 수정`), 이모지.
- **언어**: 대상 파일의 기존 주석 언어를 따른다 (응답 언어 정책은 채팅 응답에만 적용, 코드 주석에는 미적용).
- **완료 보고 전 자가 점검**: 추가·수정한 모든 주석이 위 규칙과 skill의 주석 섹션 전체(두괄식/BLUF, 같은 편집에서 갱신)를 통과 — 위반은 실패한 테스트와 동일하게 취급한다.

## 정책 라우팅 (DRY — 정책마다 단일 출처)

상세 정책은 전부 **Skill**이다: 트리거될 때만 로드되어 always-on 컨텍스트를 가볍게 유지한다. 상황이 맞으면 skill(또는 `/command`)을 호출한다.
