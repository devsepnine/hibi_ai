...
- **주석은 두괄식**: 첫 줄에 요점 한 문장 → 그다음 상세. 무엇이 아니라 왜, 코드 나열식 서술 금지, 낡은 주석 금지. 주석 언어는 대상 파일의 기존 주석 언어를 따른다 (위 응답 언어 정책은 채팅 응답에만 적용되며, 코드에 쓰는 주석에는 적용되지 않는다). 상세 규칙은 `coding-standards` skill.
