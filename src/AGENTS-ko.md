...
- `commit`, `push`, `gh pr create`는 그 커밋·푸시·PR 각각에 대한 명시적 요청이 있을 때만 실행한다. 이전의 "알아서 해줘", 직전 커밋이나 푸시에 대한 승인, 승인받은 계획, 명령을 자동 승인하는 권한 모드는 그 요청이 아니다 — 트리를 그대로 두고, 무엇이 준비됐는지 보고하고, 묻는다.
- 브랜치 전략은 요청 없이 바꾸지 않는다.
