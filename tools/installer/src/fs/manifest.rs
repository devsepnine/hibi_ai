...
pub struct InstallManifest {
    /// Repository the installed config came from.
    pub source: String,
    ...
    /// Installed components as `<type>/<name>`, sorted.
...
/// Components present in the destination and produced by this project.
///
...
fn build(dest_dir: &Path, component_ids: Vec<String>, epoch_secs: u64) -> InstallManifest {
    let target = dest_dir
        .file_name()
...
mod tests {
    ...
    fn build_falls_back_to_claude_for_a_rootless_dest() {
        let manifest = build(&PathBuf::from("/"), Vec::new(), 0);
    ...
    fn write_to_creates_a_parsable_manifest_and_leaves_no_temp_file() {
        let nanos = SystemTime::now()
            ...
            .unwrap()
            .subsec_nanos();
        let dir = std::env::temp_dir().join(format!("hibi_manifest_{nanos}"));
    ...
    fn write_to_replaces_a_previous_manifest_rather_than_appending() {
        let nanos = SystemTime::now()
            ...
            .unwrap()
            .subsec_nanos();
        let dir = std::env::temp_dir().join(format!("hibi_manifest_replace_{nanos}"));
        let path = dir.join("install.json");
    ...
    fn manifest_round_trips_through_json() {
        let manifest = build(&PathBuf::from("/Users/x/.claude"), vec!["skills/a".to_string()], 0);
