...
전체 63 파일 / 11,630줄(테스트·공백 포함 raw 라인 수).
...
| `ui/` | 17 / 2,658 | 렌더링 — 리스트, diff, 탭, MCP/플러그인 목록, 소스 위저드, 로딩 화면, `help.rs`(`?` 키바인딩 오버레이 — 전체 키 목록의 SSOT), `confirm_exit.rs`(`Esc` 이탈 확인 프롬프트), `layout.rs`(오버레이 중앙 배치 헬퍼), `tests.rs`(판 테두리·타이틀·상태바·오버레이 렌더 검증) |
...
테스트: `cargo test --manifest-path tools/installer/Cargo.toml` — 131 tests.
